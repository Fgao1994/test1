@echo off
e:
cd E:\WebSocket\client\OpenMIWebSocketClient\bin
...
set /a  diff_local = ((%diffM%*60+%diffS%)*1000+%diffMS%)/%Count%
echo diffM:%diffM%,diffS:%diffS%,diffMS:%diffMS%
echo local spend:%diff_local%
...
set /a  diffM=%EndM%-%StartM%
set /a  diffMS =%EndMS%-%StartMS%
set /a  diff_http = ((%diffM%*60+%diffS%)*1000+%diffMS%)/%Count%
echo diffM:%diffM%,diffS:%diffS%,diffMS:%diffMS%
