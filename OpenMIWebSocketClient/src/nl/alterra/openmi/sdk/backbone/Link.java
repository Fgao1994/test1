...
public class Link extends BackboneObject implements ILink, Serializable {
    ...
    public enum ValidationStatus {
        ...
        Ok("Ok"),
        NullReferences("Missing component(s) and/or exchange item(s)."),
        SourceItemNotPartOfSourceComponent("Output echange item not part of source linkable component."),
        TargetItemNotPartOfTargetComponent("Input exchange item not part of target linkable component."),
        MismatchingQuantities("Source and target quantities do not match.");
     ...
     */
    public static ValidationStatus canConnect(ILinkableComponent source, IOutputExchangeItem sourceItem,
        ...
        if ((sourceQuantity == null) || (targetQuantity == null)) {
            ...
            if ((sourceQuantity instanceof Quantity) && (targetQuantity instanceof Quantity)) {
                if (!((Quantity) sourceQuantity).describesSameAs(targetQuantity)) {
                    return ValidationStatus.MismatchingQuantities;
                }
            } else {
    ...
    public boolean describesSameAs(Object obj) {
        ...
        return ((getSourceComponent() == l.getSourceComponent()) &&
                ...
                (getTargetComponent() == l.getTargetComponent()) &&
                (getTargetExchangeItem() == l.getTargetExchangeItem()));
    }
