...
public class SmartBuffer implements Serializable {
    ...
    public IValueSet getValues(ITime requestedTime) throws Exception {
        ...
        //add by zmd 20150318, record the provenance detail
        Logger logger = Logger.getLogger(this.getClass());
        if (requestedTime instanceof TimeStamp) {
        	Calendar calendar;
        	String timestr;
        	String buf = "[";
        	for(int i=0;i<times.size();i++){
        		 calendar = CalendarConverter.modifiedJulian2Gregorian(((TimeStamp)times.get(i)).getModifiedJulianDay());
            	 timestr = OpenMIUtilities.calendar2Str(calendar, "yyyy-MM-dd HH:mm:ss");
        		buf +=" ("+timestr+","+values.get(i)+") ";
        	}
        	buf+="]";
        	 calendar = CalendarConverter.modifiedJulian2Gregorian(((TimeStamp)requestedTime).getModifiedJulianDay());
        	 timestr = OpenMIUtilities.calendar2Str(calendar, "yyyy-MM-dd HH:mm:ss");
        	 buf = "\nProvenance-"+timestr+",result-"+((ScalarSet)returnValueSet).get(0)+":\n"+buf;
        	 logger.info(buf);
