...
public class SmartOutputLinkSet extends SmartLinkSet {
    ...
    public SmartBufferInfo getSmartBuffer(String linkid){
    	int matchLinkNumber = -1;
        for (int i = 0; i < smartLinkList.size(); i++) {
            if (((SmartOutputLink) smartLinkList.get(i)).link.getID().equals(linkid)) {
                matchLinkNumber = i;
        ...
        SmartBuffer smartBuffer = ((SmartOutputLink) smartLinkList.get(matchLinkNumber)).getSmartBuffer();
        SmartBufferInfo bufferInfo = new SmartBufferInfo();
        
        //调用复制构造函数
        bufferInfo.setSmartBuffer(new SmartBuffer(smartBuffer));
        bufferInfo.setLinkId(linkid);;
        return bufferInfo;
    }
    /**
